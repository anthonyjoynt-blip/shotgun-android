...
- **Data model** (`model/Models.kt`): `Player`, `GameDef`, `ScoreEvent`.
  Scores are never stored directly â€” the scoreboard is always the sum of
...
- **Screens**: Game Library â†’ Player Setup â†’ *game* â†’ Scoreboard.
  `NavGraph.kt` dispatches on `GameType`, one screen per type:
  - **Guess It** (`GUESS20`) â€” pick a guesser, 20 yes/no questions, fewer
    questions = more points.
  - **Punch Tally** (`TALLY`) â€” one card per rider, tap to add one, âˆ’ to undo.
  - **Alphabet Hunt** (`SEQUENCE`) â€” each rider hunts Aâ†’Z in order; the card
    shows the next letter they need.
  - **Categories** (`ELIMINATION`) â€” pick a category, five-second turns,
    the clock eliminates on its own, last rider standing wins.
  - **Two Truths, One Lie** (`POINTS`) â€” pick a teller, everyone else votes
    on the lie; +1 for spotting it, +1 to the teller per rider fooled.
  - **Plate Bingo** (`BINGO`) â€” a shuffled 3Ã—3 card of plate spots per rider;
...
- **Theme**: coral/teal/gold/navy palette (`ui/theme/Color.kt`); Bebas Neue
  for display text and Inter for body, bundled in `res/font/` (both SIL OFL â€”
...
2. Write one screen that takes `(game, session, onFinished)` and calls
   `session.addScore(...)` as play happens â€” see `TallyScreen.kt` for the
   smallest example.
